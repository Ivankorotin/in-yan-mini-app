...
export async function authenticateWithTelegram() {
  ...
  let response
  try {
    response = await fetch(`${url}/functions/v1/telegram-auth`, {
      method: 'POST',
      headers: {
    ...
    return { participantId: null, reason: 'function-error', message: `сеть: ${networkErr.message}` }
  }
  ...
  if (!response.ok || !data?.token) {
    ...
    const message = `HTTP ${response.status} — ${data?.message || data?.error || rawText || 'пустой ответ'}`
